/// @return "Config JSON" string that encodes the struct/array nested data
/// 
...
function SnapBufferWriteConfigJSON(_buffer, _value, _pretty = false, _alphabetise = false, _accurateFloats = false)
{
    return __SnapToConfigJSONBufferValue(_buffer, _value, _pretty, _alphabetise, _accurateFloats, "");
}

function __SnapToConfigJSONBufferValue(_buffer, _value, _pretty, _alphabetise, _accurateFloats, _indent)
{
    ...
    else if (is_array(_value))
    {
        ...
        {
            var _firstElement = _array[0];
            if (is_struct(_firstElement))
            {
                if ((_count == 1) && (variable_struct_names_count(_firstElement) == 0))
                {
                    buffer_write(_buffer, buffer_u16, 0x7D7B); //Open then close curly bracket
                }
                else
                {
                    if ((variable_struct_names_count(_firstElement) != 2) || !variable_struct_exists(_firstElement, "k") || !variable_struct_exists(_firstElement, "v"))
                    {
                        show_error("SNAP:\nMalformed struct-array\n ", true);
                    }
                    ...
                    if (_pretty)
                    {
                        ...
                        repeat(_count)
                        {
                            var _definition = _array[_i];
                            var _name = _definition.k;
                            
                            if (!is_string(_name)) show_error("SNAP:\nKeys must be strings\n ", true);
                            
                            buffer_write(_buffer, buffer_text, _indent);
                            __SnapToConfigJSONBufferValue(_buffer, _name, _pretty, _alphabetise, _accurateFloats, _indent);
                            buffer_write(_buffer, buffer_u16,  0x203A); // <: >
                            
                            __SnapToConfigJSONBufferValue(_buffer, _definition.v, _pretty, _alphabetise, _accurateFloats, _indent);
            ...
            else
            {
                if (_pretty)
                {
                    ...
                    {
                        buffer_write(_buffer, buffer_text, _indent);
                        __SnapToConfigJSONBufferValue(_buffer, _array[_i], _pretty, _alphabetise, _accurateFloats, _indent);
                        buffer_write(_buffer, buffer_u16, 0x0A2C); //Comma + newline
                        ++_i;
                    }
        ...
        else
        {
            if (_pretty)
            {
                ...
                repeat(_count)
                {
                    ...
                    __SnapToConfigJSONBufferValue(_buffer, _struct[$ _name], _pretty, _alphabetise, _accurateFloats, _indent);
